...
enum ReapDecision {
    ...
    Skip(&'static str),
    /// Holder is provably a stale pre-update instance — reap this pid.
    Reap { pid: i32 },
}
...
/// machine's hostname (`None` if unresolved — then the host gate is skipped and
/// the marker requirement alone protects us).
fn decide_reap(
    ...
    if !marker_fresh {
        // The decisive safety gate: without a recent post-update marker a live
        // holder may be a healthy running instance. Defer to the preflight wait
        // rather than risk killing it.
        return ReapDecision::Skip("no recent post-update marker; deferring to preflight wait");
    }
    ReapDecision::Reap { pid }
}
...
mod imp {
    ...
    use super::{
        cef_cache_path, decide_reap, marker_is_fresh, marker_path_for, ReapDecision, MARKER_MAX_AGE,
        TERM_GRACE,
    };
    ...
    pub(crate) fn reap_stale_cef_lock_holder() {
        ...
        match decide_reap(holder, self_pid, local_host.as_deref(), marker_fresh) {
            ...
            ReapDecision::Reap { pid } => reap_pid(&lock_path, pid),
        }
    }
    ...
    fn reap_pid(lock_path: &Path, pid: i32) {
        let pid_u32 = pid as u32;
        log::warn!(
            "[cef-stale-reap] recent post-update marker + live CEF-lock holder pid={pid}; \
             treating as wedged pre-update instance, sending SIGTERM"
        );
        ...
        if holder_still_owns_lock(lock_path, pid) {
            log::warn!(
    ...
    /// Read the marker's freshness and delete it (one-shot). Returns `true`
    /// only if the marker existed and is within [`MARKER_MAX_AGE`].
    fn consume_marker(marker_path: &Path) -> bool {
        let fresh = match std::fs::metadata(marker_path) {
            Ok(meta) => {
                ...
                log::info!(
                    "[cef-stale-reap] update-relaunch marker present at {} (fresh={fresh})",
                    marker_path.display()
                );
                fresh
            }
    ...
    mod tests {
        ...
        fn holder_still_owns_lock_matches_same_pid_only() {
            ...
            assert!(holder_still_owns_lock(&lock, me));
            // A different (dead) pid must not re-validate.
            assert!(!holder_still_owns_lock(&lock, 2147483646));
            let _ = std::fs::remove_dir_all(&tmp);
        }
        ...
        fn consume_marker_reports_fresh_and_deletes() {
            ...
            assert!(consume_marker(&marker), "just-written marker must be fresh");
            assert!(
                std::fs::metadata(&marker).is_err(),
                "marker must be deleted after consume"
            );
            // Second consume: absent → not fresh, no error.
            assert!(!consume_marker(&marker));
            let _ = std::fs::remove_dir_all(&tmp);
        }
        ...
        fn write_then_consume_roundtrip_via_env() {
            ...
            assert!(consume_marker(&marker), "freshly written marker is fresh");
...
mod tests {
    ...
    fn skip_when_no_holder() {
        assert_eq!(
            decide_reap(None, 100, Some("host"), true),
    ...
    fn skip_when_holder_is_self() {
        assert_eq!(
            decide_reap(Some(("host".into(), 100)), 100, Some("host"), true),
    ...
    fn skip_when_holder_on_other_host() {
        assert_eq!(
            decide_reap(Some(("otherbox".into(), 200)), 100, Some("host"), true),
    ...
    fn skip_when_no_fresh_marker_even_if_live_holder() {
        ...
        assert_eq!(
            decide_reap(Some(("host".into(), 200)), 100, Some("host"), false),
    ...
    fn reap_only_with_marker_matching_host_and_other_pid() {
        assert_eq!(
            decide_reap(Some(("host".into(), 200)), 100, Some("host"), true),
            ReapDecision::Reap { pid: 200 }
        );
    }

    #[test]
    fn reap_when_local_host_unresolved_but_marker_fresh() {
        // Unresolved local host skips only the host gate; the marker gate still
        // governs, so a fresh marker still permits the reap.
        assert_eq!(
            decide_reap(Some(("host".into(), 200)), 100, None, true),
            ReapDecision::Reap { pid: 200 }
        );
    }
