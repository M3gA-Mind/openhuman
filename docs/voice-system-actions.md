...
>
> **Status:** the Rust inner loop (M1), poll-until-stable settle (M2), Music fast-path (M3, proven live), notch progress streaming (M4), the richer element model (M5), the **vision fallback for Electron/partial-AX apps** (Change 1.16), and the **browser fast-path + keyboard-shortcut driving** (Change 1.17) are all shipped. (Per-app native fast-paths now cover **Music** and **browsers**; Spotify/Slack remain **descoped** — the general model-driven loop covers them, so a deterministic accelerator isn't worth the per-app maintenance right now.)
...
### Change 1.17 — browser fast-path + keyboard-shortcut driving ✅ Done

**Status:** ✅ Shipped. Motivated by a live transcript of *"open my Brave browser, go to youtube.com and play a music video"* (analysed from `session_raw`): the `desktop_control_agent` was re-delegated **6×** (each sub re-launched Brave + re-navigated), ran `ax_interact` on the **wrong app names** (`Google Chrome`/`Chrome`/`Safari` — never the real `Brave Browser`, because Chromium exposes no AX tree), then "played" via a **blind hardcoded mouse click at (400,350)** with no verification.

**Two fixes — a deterministic browser accelerator and first-class keyboard-shortcut driving:**

- **`src/openhuman/accessibility/app_fastpaths/browser.rs` (new)** — consulted by `automate::run` before the model loop (same contract as `music.rs`). Pure parsers + `run(app, goal, backend)`:
  - `resolve_browser(app, goal)` — maps aliases → macOS display names (`brave`→`Brave Browser`, `chrome`→`Google Chrome`, `edge`→`Microsoft Edge`, `safari`/`firefox`/`arc`). Checks the `app` arg first, then the goal; a generic "browser" with no named product → **`None`** so it never guesses the wrong app (the original bug).
  - `extract_destination(goal)` — YouTube intent → `https://www.youtube.com/results?search_query=…` (percent-encoded); Google/"search for X" → `google.com/search?q=…`; a bare domain/URL ("go to example.com") → normalized to `https://…`.
  - **Navigation is one deterministic step:** `backend.open_url_in_app(browser, url)` → `open -a "<browser>" "<url>"` launches/foregrounds + navigates with no address-bar typing and no AX.
  - **Pure-navigation goals complete here** (success). **Play goals** navigate to the search-results URL then return **non-success** so the general loop does the single "click first result" via `vision_click` (no native shortcut *selects* a search result — we don't fake success). **In-page media control** ("pause/next/fullscreen the video") sends the YouTube shortcut (`k`/`shift+n`/`f`/`m`) directly.
- **Keyboard shortcuts as a first-class lever:**
  - Extracted shared `pub(crate)` helpers `run_hotkey`/`run_key`/`run_type_text` from `tools/impl/computer/keyboard.rs` (single place for validation + main-thread enigo dispatch, Change 1.15).
  - New `AutomateBackend` methods (non-breaking defaults): `open_url_in_app`, `key`, `type_text`. `RealBackend` impls `open -a` per-app and delegates keystrokes to the keyboard helpers.
  - New **`hotkey` verb** in the general `automate` loop + system prompt (`{"action":"hotkey","keys":["Cmd","L"]}`), so the model can use *any* app's shortcuts (focus address bar, `/` YouTube search, `k` play/pause) instead of hunting AX labels — folded into the no-progress signature.
  - **Cross-platform shortcut table** `app_fastpaths/browser_shortcuts.rs` — `shortcut(intent, browser, os)` resolves browser-level intents (address bar, new/close/reopen tab, tab N, next/prev tab, find, reload/hard-reload, back/forward, history, downloads, zoom, private window, fullscreen) to the right chord per **Chrome/Firefox/Safari/Edge × macOS/Windows/Linux**. The four browsers are nearly uniform (only ⌘↔Ctrl differs); the table encodes the real exceptions (Firefox-Linux `Alt+1‑8`, Firefox private window `⌘/Ctrl+Shift+P`, per-browser History/Downloads, Mac `⌘[`/`]` back-forward, `F11` vs `⌃⌘F` fullscreen). Sourced from the official Chrome/Firefox/Edge/Safari shortcut docs. `browser.rs` parses browser-level commands ("open a new tab", "go back", "reload", "switch to tab 3", "show history") and dispatches the resolved chord via `backend.key` — deterministic, no AX. (Brave/Arc/Chromium map to the Chrome family.)
  - **Artist-aware Music verification** (`music.rs` + `now_playing` backend method) — live fix after "play Numb by Linkin Park" played a *same-titled song by the wrong artist* and the agent over-claimed success. Apple Music's AX row label is **title-only** ("Numb - Single"), so a title match can land on the wrong artist. `music.rs` now parses the requested artist (`extract_artist`, "…by Linkin Park"), and after Play reads the **now-playing name + artist** via AppleScript (`AutomateBackend::now_playing` → `osascript … get {name, artist} of current track`) and compares. On a match the summary names the verified track + artist; on a **mismatch it says so honestly** ("Now playing 'Numb' by 'Tom Odell' — couldn't find … by 'Linkin Park'…") instead of claiming the requested track. Closes the wrong-artist false-success. **Follow-up:** better result *selection* (correcting to the right artist) is limited by AX `press`-by-label only hitting the first same-titled row — a library AppleScript `play (track whose name … and artist …)` fallback is the next step.
  - **Actionable `automate` responses** (from 3-transcript review) — the tool's replies were *honest but inert* (described the end state without a next move), so the agent re-delegated the same failing search 6×. Four fixes: **(1)** terminal failures (`music.rs` no-match/mismatch, `automate.rs` budget-exhausted/no-progress) now carry **next-step guidance + a "don't repeat this" steer**; **(2)** they **list the actual candidates / on-screen labels** seen (`candidate_labels`, `screen_hint`) instead of a bare negative; **(3)** **verified-only success** — never "Playing 'X'" unless `now_playing` confirms it, and an artist/album press is reported as "navigated, no track started" rather than success; **(4)** `pick_row` now **prefers real song rows** (`AXCell`/`AXRow`) over artist/album buttons (the live bug pressed the "LINKIN PARK" artist row) and the step log names the element *type* pressed. Covered by scripted-backend tests (candidate listing, wrong-artist steer, non-song honesty, song-row preference, budget/no-progress on-screen hints).
  - **App-shortcut fast-path** `app_fastpaths/app_shortcuts.rs` — same model for **Spotify, Apple Music, and Slack**: `shortcut(intent, app, os)` plus a goal parser, dispatched from `try_fastpath` after `music`/`browser`. Drives transport/navigation by the app's own global shortcut — *much* faster and more reliable than the AX tree. Media intents (play/pause, next, prev, volume ±, mute, shuffle, repeat, search) for Spotify + Apple Music; Slack navigation (quick-switcher, search, new message/compose DM, next/prev unread, next/prev channel, threads, all-unread, mark read). Encodes the real quirks: **Spotify uses ↓/↑ for next/previous** (not ←/→); **Apple Music prefixes everything with Ctrl on Windows** (Ctrl+Space vs bare Space on Mac); Slack channel-nav uses **Option/Alt+arrows**. Intents with no simple chord on a platform (Spotify volume, Apple Music mute, Apple Music Windows search access-key) return `None` so the loop falls through. Complementary to `music.rs`, which still owns Apple Music *song search/play* (it types a query); this module is transport control. **Next step (not done):** wire this table into the Phase 3 voice `command_router` so "pause / next / volume up" route to the frontmost media app's shortcut per-OS.

**Net for the example:** `open -a "Brave Browser" "https://www.youtube.com/results?search_query=music%20video"` + one `vision_click` — instead of 6 delegations and 5 failed AX calls.

**Tests:** parser units (browser resolution incl. Brave + generic→None; YouTube/Google/bare-domain destination + percent-encoding; media-control mapping; whole-word `play` guard); scripted-backend sequence (model-free nav success, play→fall-through, media-control hotkey, `try_fastpath` dispatch); an `automate` loop test that the `hotkey` verb reaches `backend.key`; keyboard tool tests unchanged after the extraction; `#[ignore]` macOS live nav test. **Follow-up:** wiring browser/YouTube shortcuts into the Phase 3 voice `command_router` (voice "pause"/"next" → browser hotkeys when a browser is frontmost).
...
| 1.5 | Browser fast-path + keyboard-shortcut driving (Change 1.17) | ✅ Done (`browser.rs`: deterministic `open -a "<browser>" url` nav, play→fall-through to vision_click; new `hotkey` verb + `key`/`type_text`/`open_url_in_app` backend methods) |
