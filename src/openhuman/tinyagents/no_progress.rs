//! Foundational no-progress primitive (issue #4089).
//!
...
//!
//! It is deliberately free of `tinyagents` harness types so it can be unit
//! tested in isolation and reused by the reliability work layering on top of it
//! (self-suspend poll loops #4090, escalate-after-N-iterations #4092). The
//! [`RepeatedToolFailureMiddleware`](super::middleware::RepeatedToolFailureMiddleware)
//! is its first driver: it feeds each tool outcome in and turns the returned
//! [`NoProgress`] verdict into a steering `InjectMessage` (nudge) or `Pause`
//! (halt).
...
/// never succeed. Matches the legacy progress-guard tolerance.
...
/// failing means the goal is unreachable here. Legacy `NO_PROGRESS_FAILURE_THRESHOLD`.
...
/// call re-issued unchanged can never succeed. Legacy `HARD_REJECT_REPEAT_THRESHOLD`.
...
pub struct ToolAttempt<'a> {
    /// Tool name.
    pub tool: &'a str,
    /// Stable fingerprint of the call arguments (see
    /// [`args_fingerprint`](super::middleware::args_fingerprint)). Folded into
    /// the identical-repeat signature so the "identical arguments" ladder only
    /// trips when the args truly repeat.
...
/// repeated failure — the core #4089 case (same tool, same args, same error).
...
/// retain the deterministic leading detail the model/user needs).
pub(super) fn truncate_for_halt(text: &str) -> String {
    const MAX: usize = 600;
